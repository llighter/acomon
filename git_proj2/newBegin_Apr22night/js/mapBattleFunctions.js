...
function WorldPokemon(id, name, lv, exp, hp, att, property){
	this.id = id;
	this.name = name; 
	this.lv = lv; 
	this.exp = exp; 
...
// 포켓몬세계 전체 몬스터리스트.
mapPokemons = [];
mapPokemons.push(new WorldPokemon(0, "jbMon", 1, 0, 50, 13, 0));  // 세계 몬스터도감 id는 순서대로 넣어줄것.
mapPokemons.push(new WorldPokemon(1, "ynMon", 1, 0, 52, 12, 1)); 
mapPokemons.push(new WorldPokemon(2, "gyMon", 1, 0, 54, 11, 2)); 
mapPokemons.push(new WorldPokemon(3, "jwMon", 1, 0, 56, 10, 3)); 
mapPokemons.push(new WorldPokemon(4, "jhMon", 1, 0, 57, 9, 4)); 	//property: "물","바람","풀","불","흙"
mapPokemons.push(new WorldPokemon(5, "bossMon", 1, 0, 57, 9, 4));   
		// ### 맵팀: 보스는 흙속성이니깐 물속성에 약함 >> 퀘스트줄때 힌트제공?
...
var bookNumber = 0;
function MyPokemon(bookNo, id, name, lv, exp, hp, att, property, status){
	this.bookNo = bookNo;
	this.id = id;
	this.name = name; 
	this.lv = lv; 
...
//								몬북고유번호지정,id,  name ,lv,exp,hp,att,상성,현재상태
pokemons.push(new MyPokemon((bookNumber++), 2, "gyMon", 1, 0, 54, 17, 2 ,0 ));  // 일반공격이 1이면 스킬공격은 최소 1.2 최대1.7랜덤
pokemons.push(new MyPokemon((bookNumber++), 3, "jwMon", 1, 0, 56, 15, 3 ,0 ));  //  pokemons[].id
pokemons.push(new MyPokemon((bookNumber++), 1, "ynMon", 1, 0, 52, 15, 1 ,0 ));  // 포켓몬번호 이름 체력 기본공격력 기본방어력
pokemons.push(new MyPokemon((bookNumber++), 4, "jhMon", 1, 0, 58, 14, 4 ,0 ));  
pokemons.push(new MyPokemon((bookNumber++), 0, "jbMon", 1, 0, 50, 13, 0 ,0 ));  
//mapPokemons    // id:0, name:"aa", lv:1, exp:0, hp: 50, att:13, property:"물"
// jb < yn < gy < jw < jh < jb  ... 먹이사슬 외에는 평범하게고우.

//#### 맵팀: 야생 포켓몬과의 만날때 시작부분.
...
function checkPokemonBook(bookNumber){  // bookNumber = listCount-1;
	...
	console.log("skill: "+ pokemons[bookNumber].status);
	console.log("=================");

	
	// function check jiwooBag(){     //###가방보기는 이런식으로 뗴어내면 됨.
...
function store(wantedService){
	...
	if(wantedService == "mint" && jiwoo.golds >= 3000){   //### 사실 금고에 돈이 잇으면 알아서 뽑아가게해도되는데..
		...
		for(var inx =0; inx< pokemons.length; inx++){
			storeMsg += "\n" + pokemons[inx].name + ": "+ pokemons[inx].hp;
